...
export default function Card8() {
  ...
  const fetchDataFromFirebase = async () => {
    try {
      const snapshot = await firebase.firestore().collection('sentences').doc('phishing').get();
  ...
  return (
    ...
    marginTop: 10, borderWidth: 1.5, borderColor: '#728c93', borderRadius: 5, backgroundColor: '#ebf8fc'}}>
      <Image source={SEImage2} style={{width: 40, height: 40, marginLeft: 10 }} />
        <Text style={{fontSize: 15, fontWeight: "bold", color: "#333", marginLeft: 17}}>PHISHING</Text>
    </View>
