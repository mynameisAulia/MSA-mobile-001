...
export default function Card7() {
  ...
  const fetchDataFromFirebase = async () => {
    try {
      const snapshot = await firebase.firestore().collection('sentences').doc('pretext').get();
  ...
  return (
    ...
    <View style={{ width: 165, height: 55, flexDirection: "row", alignItems: 'center', 
    marginTop: 10, borderWidth: 1.5, borderColor: '#728c93', borderRadius: 5, backgroundColor: '#ebf8fc'}}>
      <Image source={SEImage8} style={{width: 40, height: 40, marginLeft: 10 }} />
        <Text style={{fontSize: 15, fontWeight: "bold", color: "#333", marginLeft: 17}}>PRETEXTING</Text>
    </View>
      ...
      <View style={{ width: width, flexDirection: 'row', marginTop: 10, justifyContent: 'center' }}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('Video7')}
        >
          <Text 
          style={{ fontWeight: "bold"}}
          >Watch Video</Text>
