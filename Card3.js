...
export default function Card3() {
  ...
  const fetchDataFromFirebase = async () => {
    try {
      const snapshot = await firebase.firestore().collection('sentences').doc('pharm').get();
  ...
  return (
    ...
    <View style={{ width: 155, height: 55, flexDirection: "row", alignItems: 'center', 
    marginTop: 10, borderWidth: 1.5, borderColor: '#728c93', borderRadius: 5, backgroundColor: '#ebf8fc'}}>
      <Image source={SEImage7} style={{width: 40, height: 40, marginLeft: 10 }} />
        <Text style={{fontSize: 15, fontWeight: "bold", color: "#333", marginLeft: 17}}>PHARMING</Text>
    </View>
      ...
      <View style={{ width: width, flexDirection: 'row', marginTop: 10, justifyContent: 'center' }}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('Video10')}
        >
          <Text 
          style={{ fontWeight: "bold"}}
          >Watch Video</Text>
